...
class PrometheusListener
  def initialize(registry = Prometheus::Client.registry)
    @log_count_metric = registry.gauge(
      :barito_market_log_count, docstring: 'Count of consumed logs', labels: %i[app_group app_name environment]
    )
    @log_throughput_metric = registry.gauge(
      :barito_market_log_throughput, docstring: 'Log throughput', labels: %i[app_group app_name environment]
    )
  ...
  def log_count_changed(app_id, app_log_throughput)
    ...
    @log_count_metric.set(app.log_count, labels: labels)
    @log_throughput_metric.set(app_log_throughput, labels: labels)
  end
