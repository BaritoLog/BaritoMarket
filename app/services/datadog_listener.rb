class DatadogListener
  ...
  def log_count_changed(app_id, app_log_throughput)
    ...
    @dog.batch_metrics do
      @dog.emit_point(
        "barito.app_group.log_count", app_group.log_count, :app_group => "#{app_group.name}", :app_name => "#{app.name}")
      @dog.emit_point(
        "barito.app_group.log_throughput", app_log_throughput, :app_group => "#{app_group.name}", :app_name => "#{app.name}")
    end
  end
