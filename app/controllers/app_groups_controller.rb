...
class AppGroupsController < ApplicationController
  ...
  def update
    ...
    if params['helm_infrastructure_id'].present?
      @app_group.helm_infrastructures.find(params['helm_infrastructure_id']).update_attributes(helm_infrastructure_params)
    end
