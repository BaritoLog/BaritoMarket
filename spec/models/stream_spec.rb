...
RSpec.describe Stream, type: :model do
  ...
  context 'name' do
    it 'must be presence' do
      group = FactoryBot.build(:stream, name: '')
  ...
  context 'receiver_host' do
    it 'must be presence' do
      group = FactoryBot.build(:stream, receiver_host: '')
      expect(group).to_not be_valid
    end
  end
  ...
  context 'zookeeper_hosts' do
    it 'must be presence' do
      group = FactoryBot.build(:stream, zookeeper_hosts: '')
      expect(group).to_not be_valid
    end
  end
  ...
  context 'kafka_broker_hosts' do
    it 'must be presence' do
      group = FactoryBot.build(:stream, kafka_broker_hosts: '')
      expect(group).to_not be_valid
    end
  end
  ...
  context 'kafka_topic_partition' do
    it 'must be presence' do
      group = FactoryBot.build(:stream, kafka_topic_partition: nil)
      expect(group).to_not be_valid
    end

    it 'greater than or equal to kafka broker host number' do
      group = FactoryBot.build(:stream, kafka_broker_hosts: 'host,host2', kafka_topic_partition: 1)
      expect(group).to_not be_valid
    end
  end
  ...
  context 'receiver_port' do
    it 'must be presence' do
      group = FactoryBot.build(:stream, receiver_port: '')
      expect(group).to_not be_valid
    end
  end
  ...
  context 'when group created' do
    it 'create receiver config' do
      group = FactoryBot.create(:stream)
